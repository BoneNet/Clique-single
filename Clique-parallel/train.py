...
def get_available_gpus():
    ...
    return [i.name for i in local_devices if i.device_type == "GPU"]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--img_size', default=128,
                        help='img_size used to resized the img')
    ...
    train_data, train_label, test_data, test_label = load_data(img_size)
    ...
    with graph.as_default():
        ...
        devices = ['/gpu:0', '/gpu:1']
        if len(devices) < 2: devices.append(devices[0])
        if len(devices) < 3: devices.insert(1, devices[0])
        if len(devices) < 4: devices.append(devices[2])
