...
def build_model(input_images, k, T, label_num, is_train, keep_prob, if_a, if_b, if_c, devices):
    ...
    for i in range(block_num):
        with tf.device(devices[2]):
            block_feature, transit_feature = loop_block(current, if_b, channels_per_layer=k, layer_num=T / 3,
                                                        is_train=is_train, keep_prob=keep_prob, block_name='b' + str(i))
        if if_c == True:
            with tf.device(devices[2]): block_feature = compress(block_feature, is_train=is_train, keep_prob=keep_prob,
                                                                 name='com' + str(i))
        with tf.device(devices[2]):
